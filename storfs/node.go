...
import (
	...
	// namesys "github.com/ipfs/kubo/namesys"
...
func initializeGateway(node *core.IpfsNode) error {
	...
	// listeners, err := sockets.TakeListeners("gateway.ipfs.io")
	// listeners, err := sockets.TakeListeners("gateway.ethoprotocol.com")
	// listeners, err := sockets.TakeListeners("com.ethoprotocol.gateway")
	listeners, err := sockets.TakeListeners("com.ethoprotocol.data")
	if err != nil {
		return fmt.Errorf("serveHTTPGateway: socket activation failed: %s", err)
	}

	log.Info("listeners", "listeners", listeners)
	log.Info("listeners", "listeners", listeners)
	log.Info("listeners", "listeners", listeners)
	...
	for _, addr := range gatewayAddrs {
		...
		if err != nil {
			...
			return fmt.Errorf("serveHTTPGateway: ma..Listen(%s) failed: %s", gatewayMaddr, err)
		}
...
func initializeEthofsNode(nodeType string) (icore.CoreAPI, *core.IpfsNode) {
	...
	bootstrapNodes := []string{
		...
		// "/ip4/23.94.202.163/tcp/4001/p2p/12D3KooWCppbj9caRsPNyb3Jr9yPQr9U1zLyQcd6omeGRX2GoU4R", // gateway.ethoprotocol.com
		// "/ip4/testnetgateway.ethoprotocol.com/tcp/4001/p2p/12D3KooWB4XpigvLkHVfVtyrCJJSHdHpLYLqXn7d8HZRgzKDmwCU", // testnetgateway.ethoprotocol.com
		"/ip4/173.212.208.215/tcp/4001/p2p/12D3KooWB4XpigvLkHVfVtyrCJJSHdHpLYLqXn7d8HZRgzKDmwCU", // testnetgateway.ethoprotocol.com
	}
